...
def test_omnicorp_overlay(famcov_new):
    ...
    response = client.post('/omnicorp_overlay', json=famcov_new)

    # load the json
    answer = response.json()

    # make sure the there are 27 results
    assert(len(answer['message']['results']) == 27)

    # assert there are node bindings
    assert(len(answer['message']['results'][0]['node_bindings'])== 3)

    # assert there are node bindings
    assert(len(answer['message']['results'][0]['edge_bindings']) == 6)
